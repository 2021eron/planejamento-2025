...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeAppLogic(userRole, userEmail) {
        ...
        const modalExtraHoursObs = document.getElementById('modal-extra-hours-obs'); // NOVO
        const modalLateHours = document.getElementById('modal-late-hours');
        const modalLateHoursObs = document.getElementById('modal-late-hours-obs'); // NOVO
        const modalSaveBtn = document.getElementById('modal-save-btn');
        const modalCancelBtn = document.getElementById('modal-cancel-btn');
        let currentlyEditingDate = null;

        // --- TOOLTIP ---
        const tooltip = document.getElementById('day-tooltip'); // NOVO
                ...
                if (doc.exists) {
                    ...
                    if (isEditor) {
                        ...
                        state.funcionarios.push({
                            id: initialId, nome: "Meu Planejamento", email: userEmail, calendario: {}
                        });
                        state.funcionarioSelecionadoId = initialId;
                        saveState();
                    }
                }
        ...
        function calculateAndDisplaySummaries() {
            ...
            if (funcionarioAtual && funcionarioAtual.calendario) {
                for (const dateString in funcionarioAtual.calendario) {
                    ...
                    totalExtras += parseFloat(data.extras) || 0;
                    totalAtrasos += parseFloat(data.atrasos) || 0;
                }
            }
            ...
            MONTHS.forEach((monthName, monthIndex) => {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    if (funcionarioAtual && funcionarioAtual.calendario && funcionarioAtual.calendario[dateString]) {
                        ...
                        // ATUALIZADO: O ponto (classe 'has-hours') aparece se tiver cor, horas ou observação
                        if (data.cor !== 'default' || (parseFloat(data.extras) || 0) > 0 || (parseFloat(data.atrasos) || 0) > 0 || data.extrasObs || data.atrasosObs) {
        ...
        function handleDayClick(event) {
            ...
            const data = funcionario?.calendario?.[currentlyEditingDate] || {};
            ...
            modalExtraHoursObs.value = data.extrasObs || ''; // NOVO
            modalLateHours.value = data.atrasos || '';
            modalLateHoursObs.value = data.atrasosObs || ''; // NOVO
        ...
        function handleSaveDay() {
            ...
            const newExtrasObs = modalExtraHoursObs.value.trim(); // NOVO
            const newAtrasos = parseFloat(modalLateHours.value) || 0;
            const newAtrasosObs = modalLateHoursObs.value.trim(); // NOVO

            const isDefault = newColor === 'default' && newExtras === 0 && newAtrasos === 0 && !newExtrasObs && !newAtrasosObs;
            ...
            if (isDefault) {
                ...
                funcionario.calendario[currentlyEditingDate] = {
                    cor: newColor,
                    extras: newExtras,
                    extrasObs: newExtrasObs, // NOVO
                    atrasos: newAtrasos,
                    atrasosObs: newAtrasosObs // NOVO
                };
            }
        ...
        function showTooltip(event) {
            ...
            tooltip.style.top = `${window.scrollY + rect.top - tooltip.offsetHeight - 10}px`; // 10px acima da célula
        }
        ...
        calendarContainer.addEventListener('mouseover', showTooltip); // NOVO
        calendarContainer.addEventListener('mouseout', hideTooltip);  // NOVO
